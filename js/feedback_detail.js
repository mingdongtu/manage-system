    ...
    ajaxData:function(){   // 封装一个数据加载的方法
              ...
              $(".pagination a").each(function(index,element){
                     if($(element).hasClass('active')){
                        ...
                        this.pageData.delegateId=localStorage.getItem('delegate_no');
                        
                         $.get("/feedback/get_feedback_details",this.pageData,function(data){

                             $.Logout( $(window.parent.parent),"login.html",data)
                              $('#feedback_detail_list').html(template('feedback_detail_data',data));
                               // 分页处理
...
obj_all.ajaxData();
    ...
    $(".up_level").click(function(){
        $(window).attr("location","feedback.html");
    })
