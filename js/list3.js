...
// 分页组件：点击下一个按钮
   var obj_all={
       ...
       exchange_val:function(){   // 封装一个方法，实现select选中项目与字段的匹配
           ...
           switch(select_val){
               case '药店名称':
                   search_obj.relationPharmacyName=$.trim($(".search_content").val())
                   break;
           }
           return search_obj;
       },
       ...
       ajaxSearch:function(pageNum){
           ...
           obj.delegateId=String(localStorage.getItem("delegateId"));


           $.get('/relation/get_pharmacy_details',obj,function(data){
               ...
               if(data.errcode==0){
                   ...
                   if(data.data==null){
                       ...
                       $("#drugstore_list").addClass("hide_pagination");
                   }else{
                       $(".pagination").removeClass("hide_pagination");
                       $("#drugstore_list").removeClass("hide_pagination");
                       obj_status.status=1;
                       $('#drugstore_list').html(template('drugstore_data',data));
                       ...
                       $(".drug_nums").click(function(){
                           var index=$(this).parents('tr').index();
                           var id=data.data[index].delegateId;
                           var pharmacyCode=data.data[index].pharmacyCode;
                           localStorage.setItem('pharmacyCode',pharmacyCode);
                           $(window).attr('location','list3_detail.html');
              ...
              $(".pagination a").each(function(index,element){
                     if($(element).hasClass('active')){
                        ...
                        this.pageData.delegateId=localStorage.getItem('delegateId');
                           ...
                           $.get("/relation/get_pharmacy_details",this.pageData,function(data){
                               $.Logout( $(window.parent.parent),"login.html",data)
                              $('#drugstore_list').html(template('drugstore_data',data));
