    ...
    ajaxData:function(){   // 封装一个数据加载的方法
              ...
              $(".pagination a").each(function(index,element){
                     if($(element).hasClass('active')){
                        ...
                        this.pageData.code=localStorage.getItem('codename2');
                         this.pageData.id=parseInt(localStorage.getItem('id3'));

        $.ajax({
            url:"/drug/get_drug_DoctorDetailsList",
            data:this.pageData,
            cache:false,
            success:function(data) {

                // $.get("/drug/get_drug_DoctorDetailsList",this.pageData,function(data){
                $.Logout($(window.parent.parent), "login.html", data)
                $('#doctor_detail_list').html(template('doctor_detail_data', data));
